(ns ttc2013.flowgraphs.stmt2str
  (:use funnyqt.emf)
...
(defpolyfn stmt2str 'types.PrimitiveType
  [^org.eclipse.emf.ecore.EObject pt]
  (clojure.string/lower-case (.getName (.eClass pt))))

(defpolyfn stmt2str 'statements.LocalVariableStatement
  [lv]
...
(defpolyfn stmt2str 'references.IdentifierReference
  [ir]
  (stmt2str (eget ir :target)))

(defpolyfn stmt2str 'variables.Variable
  [v]
  (eget v :name))

(defpolyfn stmt2str 'expressions.MultiplicativeExpression
  [me]
  (let [[c1 c2] (eget me :children)]
    (str (stmt2str c1) " " (reduce-str (eget me :multiplicativeOperators))
         " " (stmt2str c2))))

(defpolyfn stmt2str 'expressions.EqualityExpression
  [ee]
  (let [[c1 c2] (eget ee :children)]
    (str (stmt2str c1) " " (reduce-str (eget ee :equalityOperators))
         " " (stmt2str c2))))

(defpolyfn stmt2str 'expressions.AdditiveExpression
  [ae]
  (let [[c1 c2] (eget ae :children)]
    (str (stmt2str c1) " " (reduce-str (eget ae :additiveOperators))
         " " (stmt2str c2))))

(defpolyfn stmt2str 'expressions.UnaryExpression
  [ue]
...
(defpolyfn stmt2str 'expressions.RelationExpression
  [re]
...
(defpolyfn stmt2str 'expressions.SuffixUnaryModificationExpression
  [se]
  (str (stmt2str (eget se :child))
       (stmt2str (eget se :operator))))

(defpolyfn stmt2str 'statements.Block
  [b]
  "\\{...\\}")

(defpolyfn stmt2str 'statements.Condition
  [c]
  "if")

(defpolyfn stmt2str 'statements.WhileLoop
  [c]
  "while")

(defpolyfn stmt2str 'statements.JumpLabel
  [l]
  (str (eget l :name) ":"))

(defpolyfn stmt2str 'statements.Break
  [b]
  (str "break"
...
(defpolyfn stmt2str 'statements.Continue
  [c]
  (str "continue"
...
(defpolyfn stmt2str 'statements.Return
  [r]
  (str "return" (when-let [rv (eget r :returnValue)]
                  (str " " (stmt2str rv)))
       ";"))

(defpolyfn stmt2str 'statements.ExpressionStatement
  [stmt]
...
(defpolyfn stmt2str 'literals.Literal
  [l]
  (type-case l
